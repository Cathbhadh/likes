...
def main():
    ...
    if access_token:
        ...
        if st.button("Load Data"):
            ...
            df_likes = pd.DataFrame({ "User": list(user_likes.keys()), "Likes": [len(posts) for posts in user_likes.values()] })
            df_comments = pd.DataFrame({ "User": list(user_comments.keys()), "Comments": list(user_comments.values()) })
